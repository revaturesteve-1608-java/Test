	...
	$scope.addPost = function(postTitle, postContent, category){
		...
		postsService.createPost(postInfo, function(response){
			...
			var append = '<div id="eachPost"><ul class="list-group">'
				                +'<li class="list-group-item"> <div id="titleDiv"><div class="col-md-8">'
				                +'<h4><a href="#post?id=' + postId + '"><b>' + postTitle + '</b></a></h4></div>'
				                +'<div class="col-md-4"><button id="deleteBtn" type="button" class="btn btn-default" aria-label="Right Align" '
				                +'ng-click="deletePost(postId)"><span class="glyphicon glyphicon-remove" aria-hidden="true">'
				                +'</span></button></div> </li><li class="list-group-item"><div><p id="thePost">'+ postContent + '</p></div></li>'
				                ...
				                +'<form><fieldset><div class="col-md-6"><input class="form-control input-sm" id="replyText" type="text" '
				                +'placeholder="reply" ng-model="userReply'+ postId + '"></div>'
				                +'<div class="col-md-6"><button id="reply" class="btn btn-primary" type="button" '
				                +'ng-click="addReply(userReply'+ postId + ', postId)"> reply</button></div></fieldset></form></li></ul></div>'
...
.service('postsService', function($http){
	...
	this.createPost = function(postInformation, callback){
		$http.post("/createPost", postInformation).then(callback)
	}
	...
	this.getMorePosts = function(firstResult, callback){
		$http.get("/getMorePosts", firstResult).then(callback)
	}
	
	this.createReply = function(replyInfo){
		console.log("here i am");
		$http.post("/createReply", replyInfo).then();
	}
	
	this.deletePost = function(postId, callback){
		$http.post("/deletePost", postId).then(callback);
	}
	
	this.getPostsByCategory = function(info, callback){
		$http.post("/getPostsCatProf", info).then(callback);
	}
